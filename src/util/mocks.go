...
func NewFilledInterfaceMock(mtu int, name string, macAddr string, flags net.Flags, addrs []string, isVirtualInterface bool, speedMbps int64, interfaceType string, collectVirtualInterfaces bool) *MockInterface {
	...
	if !collectVirtualInterfaces {
		ret.On("IsVirtual").Return(isVirtualInterface)
	}
	return &ret
}
