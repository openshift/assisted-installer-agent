...
var _ = Describe("Upgrade agent command", func() {
	...
	It("Succeeds if pull fails", func() {
		// This may sound weird, but it is the expected behaviour of the command: it
		// succeeds even if the image pull, and returns to the service a response indicating
		// that the upgrade failed. This is necessary because otherwise the service will
		// not process that response and will not generate the host event that informs
		// the user that the upgrade failed.
		deps.On(
			"ExecutePrivileged",
			"podman", "pull", "quay.io/my/image:v1.2.3",
		).Return("", "", 1).Once()
