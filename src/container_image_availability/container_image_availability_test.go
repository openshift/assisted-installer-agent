...
var _ = Describe("Image availability", func() {
	...
	Context("handleImageAvailability", func() {
		...
		It("failed_to_pull_before_timeout_and_keep_trying", func() {
			imageAvailabilityDependencies.On("ExecutePrivileged", generateGetCommand(defaultTestImage)...).Return("", "", 0).Once()

			for remainingSeconds := defaultTestPullTimeoutSeconds; remainingSeconds > 0; remainingSeconds-- {
				cmd := fmt.Sprintf(templatePull, defaultTestImage)
				cmd = fmt.Sprintf(templateTimeout, strconv.FormatInt(int64(remainingSeconds), 10), cmd)
				args := convertStringArryToInterfaceArray(strings.Split(cmd, " "))
				imageAvailabilityDependencies.On("ExecutePrivileged", args...).Return("", "", 1).Once()
			}
