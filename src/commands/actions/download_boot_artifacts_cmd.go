...
func run(infraEnvId, downloaderRequestStr, caCertPath string) error {
	...
	if err := createBootLoaderConfig(*req.RootfsURL); err != nil {
...
func createBootLoaderConfig(rootfsUrl string) error {
...
func calculateBootArtifactsSize(folders *folders) (uint64, error) {
...
func moveFilesToBootFolder(folders *folders) error {
	...
	if err := os.WriteFile(path.Join(tempBootArtifactsFolder, "boot_artifacts_moved"), []byte("true"), 0600); err != nil {
